...
`ACK.ResourceSynced=True`, and the bucket appears in the S3 console with the versioning and tags
set on the Composition spec:
...
In the S3 console, the bucket's **Properties** show the region, ARN, and **Bucket Versioning:
Enabled** — straight from `spec.versioning.status`:

![Bucket Properties: region eu-central-1, ARN, and Bucket Versioning Enabled](docs/s3-quickstart-bucket-versioning.png)

…and the **Tags** include the `tagSet` from the Composition (`managed-by=krateo`,
`purpose=ack-e2e`) alongside the tags ACK adds automatically:
